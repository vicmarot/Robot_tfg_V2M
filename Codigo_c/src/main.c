...
static void Acercamiento( double *d, double Angl_Rob, double Pos[3], unsigned int destinoX, unsigned int destinoY){
  double alfa, test;
  *d=abs(sqrt(pow((destinoX-Pos[0]),2)+pow((destinoY-Pos[1]),2)));
  test==abs(sqrt(pow((destinoX-Pos[0]),2)+pow((destinoY-Pos[1]),2)));
  printf("LA DISTANCIA ES=>%lf\n",test);
  alfa=((destinoX-Pos[0])*cos(Angl_Rob)+(destinoY-Pos[1])*sin(Angl_Rob))/(*d);//calculamos alfa
  Angl_Rob=-acos(alfa)+Angl_Rob; 
...
static void Exploracion(double Pos[3], double lold[250000], double lnew[250000], double Angl_Rob){
  ...
  double ds=3.37;//=distancia entre los sensores;
  for (jf2=0;jf2<NUM_POS;jf2++){ //sus 2 por NUM_POS
    ...
    P1sx=Pos[0]+ds*sin(Pos[2]);
    P1sy=Pos[1]+ds*cos(Pos[2]);
    P2sx=Pos[0]+ds*sin(PI+Pos[2]);
    P2sy=Pos[1]+ds*cos(PI+Pos[2]);
    ...
    Pos[0]=P2sx;Pos[1]=P2sy;
    occupancy(Pos, Zt2, lnew, lnew); // pos sera la posicion del robot mas o menos la distancia del sensor dependiendo si es izquierda o derecha
    //occupancy(Pos, Zt1, lold, lnew);
...
static void Giro (double alfa, double Angl_Rob, double d){
  ...
  if (alfa>0 && acos(alfa)!=0){
   ...
   //printf("DRC EL DATO ES==>%s\n",aux );
   strcpy(str, aux);
   Com_Rasp_to_Ard(&str[MAXDATASIZE]);
   //printf("salid de com1\n");
  }
  else {
...
static void avanzar (double d){
  ...
  char aux[MAXDATASIZE]; // cambia este tamaño
  //(if d>20){
  sprintf(aux,sizeof(aux),"I%lfD%lf*",d,d); 
  //sprintf(aux,"I%lfD%lf*",d,d); 
  printf("EL DATO ES==>%s\n",aux );
  strcpy(str, aux);
  Com_Rasp_to_Ard(&str[MAXDATASIZE]);
  //}
...
static void Sigue_Pared(double d,unsigned int *destinoX, unsigned int *destinoY, double lnew[250000], double Pos[3] ){

  double dis2=30,dis=30;
  if (d<40){ // si estoy a un distancia menor que 30 de la pared
    //if ((l[ceil(rob.y+dis),ceil(rob.x)]>0.5)){ //si a la derecha esta libre y estoy siguiendo la pared. la posicion del robot mas 50 casillas en el eje < 0.5 estara libre y podemos dirigirno a ella
    if ((lnew[(int)(ceil(Pos[1]+dis)+ceil(500*Pos[0]))]>0.5)){
      if (lnew[(int)(ceil(Pos[1])+ceil(500*(Pos[0]+dis2)))]<0.5){
        *destinoX=(int)ceil(Pos[0]+dis2); //derecha
        *destinoY=(int)ceil(Pos[1]);
      }
      else if (lnew[(int)(ceil(Pos[1]-dis2)+ceil(500*Pos[0]))]<0.5){
        *destinoX=(int)ceil(Pos[0]); //subir
        *destinoY=(int)ceil(Pos[1]-dis2);
      }
    }
    else if ((lnew[(int)(ceil(Pos[1]-dis)+ceil(500*Pos[0]))]>0.5)) {
      if (lnew[(int)(ceil(Pos[1])+ceil(500*(Pos[0]-dis2)))]<0.5){
        *destinoX=(int)ceil(Pos[0]-dis2); //izquierda
        *destinoY=(int)ceil(Pos[1]);
      }
      else if (lnew[(int)(ceil(Pos[1]+dis2)+ceil(500*Pos[0]))]<0.5){
         *destinoX=(int)ceil(Pos[0]); // bajo
         *destinoY=(int)ceil(Pos[1]+dis2);
       }
    }
    else if ((lnew[(int)(ceil(Pos[1])+ceil(500*(Pos[0]+dis)))]>0.5)) {// || l(ceil(rob.y),ceil(rob.x-30))>0.5)) // si hacia abajo esta libre
      if (lnew[(int)(ceil(Pos[1]-dis2)+ceil(500*Pos[0]))]<0.5){ // subir
        *destinoX=(int)ceil(Pos[0]);
        *destinoY=(int)ceil(Pos[1]-dis2);
      }
      else if (lnew[(int)(ceil(Pos[1])+ceil(500*(Pos[0]-dis2)))]<0.5){
        *destinoX=(int)ceil(Pos[0]-dis2); // izquierda
        *destinoY=(int)ceil(Pos[1]);
      }
    }
    else if (lnew[(int)(ceil(Pos[1])+ceil(500*(Pos[0]-dis)))]>0.5){
      if (lnew[(int)(ceil(Pos[1]+dis2),ceil(Pos[0]))]<0.5){// bajo
        *destinoX=(int)ceil(Pos[0]);
        *destinoY=(int)ceil(Pos[1]+dis2);
      }
      else if (lnew[(int)(ceil(Pos[1])+ceil(500*(Pos[0]+dis2)))]<0.5){
...
printf("sigue pared: destino x:%i destino y:%i distancia:%lf\n",*destinoX,*destinoY,d);
}
...
int main(int argc, const char * const argv[])
{
 ...
 // printf("Distancia Der %lf, Distancia Izq %lf\n",Dd,Di);
  ...
  // despues de esto tiene qu rodear el objeto

  Sigue_Pared(d, &destinoX,   &destinoY,  lnew,  Pos );

  printf("Voy a cerrar el archivo\n");
